...
%{
	...
	void capitalize(char** string) { // Using this kind of functions is the only way I could find of manipulating the $ pseudo-variables.
		int i = 0;
		for (; (*string)[i] != '\0'; i++)
	...
	void printLeftAligned(const char* s, const int charsUsed) {
		...
		fprintf(yyout, "%s", s);
		newLineActions(); // New line actions are needed in the case of recursive calls of this function. A consequence of this necessity is that there will be a newline at the end of the document.
	}
...
%start file /* Sefining the start condition. Only required in the cases where the grammar does not begin with the start symbol but I included it anyway. */
...
titleProperty: TITLE LBRACE STRING RBRACE
			   {
				   ...
				   char *temp = $3; /* The temporary pointer is needed to avoid undefined behavior for attempting to modify a string literal when calling removeFirstAndLastChar(). */
				   title = temp;
